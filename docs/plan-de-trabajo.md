...
Estado al 23 de septiembre de 2026: **iteraciones 0, 2, 3, 4, 5, 6 y 7 terminadas; iteración 1 casi cerrada** (hecho el registro cargador → pod en Redis; quedan la prueba con los dos simuladores externos del laboratorio, que necesita Docker, y la suite tzi-OCTT en CI); la siguiente es la iteración 8 (infraestructura y observabilidad). De la iteración 7 quedan pendientes las compilaciones en TestFlight e Internal testing (necesitan las cuentas de Apple, Google Play y Expo del dueño) y la carga real en staging (llega con la infraestructura de la iteración 8). El repositorio contiene el diseño completo (`docs/`), este plan, el monorepo con los paquetes de dominio, esquemas OCPP, eventos, base de datos y tarifas, y las apps `ocpp-gateway`, `api` y `worker` con pruebas. Las tareas del dueño del proyecto están en `docs/tareas-del-dueno.md`. Este archivo es el plan vivo del proyecto: se actualiza al cerrar cada fase, al tomar cada decisión y cuando cambie el alcance.
...
- [x] Iteración 0: monorepo TypeScript con dominio, esquemas OCPP oficiales, eventos, migraciones, motor de tarifas (interfaz), gateway mínimo, API, worker, CI y Docker.
- [x] Iteración 1 (salvo simuladores externos y tzi-OCTT): gateway OCPP 1.6J con allowlist, Basic Auth, validación de esquemas, `BootNotification`/`Heartbeat`/`StatusNotification`, directorio cargador → pod en Redis con evicción entre pods (ADR 0013).
- [x] Iteración 2: inventario (sedes, cargadores, EVSE, conectores, plantillas), credenciales con scrypt (ADR 0014), ciclo de vida persistido con evidencia OCPP, comisionamiento (`GetConfiguration`, `ChangeConfiguration`, `Reset`, `TriggerMessage`), comandos (`ChangeAvailability`, `UnlockConnector`, `ClearCache`) registrados en `ops.command`, detección de deriva con alarma `CONFIG_DRIFT` (API y trabajo diario del worker), API de administración `/admin/v1`, simulador embebido `@volt/ocpp-sim` y prueba de aceptación de punta a punta.
- [x] Iteración 3: sesiones de carga (ADR 0016): idTag virtual de un solo uso, `RemoteStartTransaction`/`RemoteStopTransaction`, `Authorize`, `StartTransaction` idempotente con propiedad por cargador, `MeterValues` con progreso y anomalías, `StopTransaction` con duplicados y huérfanas, transacciones offline y sesiones no solicitadas, máquinas de estado de sesión, outbox con relay en el worker, expiración de arranques y cierre estimado de huérfanas, API pública `/v1` (sedes, EVSE, sesiones, SSE) y de administración, simulador con transacciones y modo offline; pruebas CU-02, CU-04, CU-05, corte de red con reintentos y caída de pod.
- [x] Iteración 4: motor de tarifas OCPI 2.2.1 puro y determinista con snapshot por sesión, costo en vivo, ocupación con gracia, tope de exposición, versiones y asignaciones de tarifa, simulador y parámetros por nivel (ADR 0011, 0012, 0017 y 0018).
- [x] Iteración 5: cobro con Wompi (ADR 0002 y 0020): fuentes de pago tokenizadas, cobro al liquidar, webhooks idempotentes, reintentos, deuda con enlace de pago y bloqueo, devoluciones, conciliación diaria y recibos; emulador para pruebas y prueba opcional contra el sandbox.
- [x] Iteración 7: app Volt (ADR 0022): identidad del conductor con Identity Platform (alta automática, vinculación segura, correo verificado y consentimientos obligatorios antes de pagar o cargar), cuenta con perfil, dispositivos, bandeja de avisos y borrado (anonimización), configuración pública de la app, tokenización de tarjeta y Nequi directo en Wompi, reto 3DS, notificaciones push desde el outbox con bandeja idempotente (Expo Push), app Expo + React Native (mapa y lista en vivo, precio desagregado, QR, carga en vivo, historial y recibos, medios de pago, cobros pendientes, avisos, es/en, marca VOLT) con versión web para pruebas.
- [x] Iteración 6: back-office (ADR 0021): identidad del personal con Identity Platform (MFA TOTP para roles privilegiados), RBAC con política central por ruta, auditoría inmutable con cadena de hashes, personal e invitaciones, resumen en vivo con mapa, cargadores con comisionamiento, configuración, comandos y bitácora, sesiones y costo, tarifas y simulador, parámetros, pagos y deudas, conductores, alarmas; español e inglés; SPA React + Vite servida por nginx.
...
| D7 | Hardware existente vs nuevo | Piloto en la semana 1; compras nuevas con certificación OCA | [x] **Equipos nuevos: 3 estaciones DC 180 kW de dos mangueras al inicio, 30 en el primer año, 40 kW de dos mangueras pronto; hay cargador para laboratorio; sin operación previa que migrar** (ADR 0003). 22-09-2026: el proveedor confirmó OCPP 1.6J y que la URL del Central System se puede cambiar al CSMS propio; el riesgo de firmware atado a la nube del fabricante queda descartado |
| D12 | Tarifa inicial e idle fee | Energía por franjas horarias; gracia y precio por minuto configurables | [x] **Energía por franja horaria con precios dinámicos después; gracia de 15 minutos (o variable por reglas) y 1.500 COP por minuto de ocupación, todo configurable** (ADR 0012). Pendiente: precios por kWh por franja |
| D8 | Perfil de seguridad objetivo | 2 desde el inicio, 3 en fase 2 | [x] **2 desde el inicio, 3 en fase 2** (ADR 0003) |
| D9 | Idiomas, marca y política de retención de datos | Español primero con i18n; retención definida por clase de dato | [x] **App "Volt"; español principal e inglés disponible; se conservan todos los datos operativos, datos personales según política de la Ley 1581** (ADR 0005); **región `us-central1`** tras la medición de latencia del 22-09-2026 (ADR 0015) |
...
| Región de Google Cloud | No existe región en Colombia (Google construye un centro de datos en Yumbo, Valle del Cauca, sin fecha de región). Decidida `us-central1` como región principal tras medir la latencia desde Bogotá el 22-09-2026 (ADR 0015); `us-east1` como región secundaria de copias de seguridad. Para OCPP la latencia es irrelevante; importa para la app. |
...
3. [x] 22-09-2026: cuenta de Wompi creada y llaves de prueba cargadas como secretos de GitHub Actions; documentación de referencia en <https://docs.wompi.co/docs/colombia/inicio-rapido/>. Pendiente: confirmar con Wompi el flujo de tokenización con 3D Secure y cobros posteriores sin presencia del cliente, y las reglas de anulación y reembolso (se hace al implementar la iteración 5).
...
6. [x] 22-09-2026: proyectos de Google Cloud creados con facturación, APIs, bucket de Terraform y Workload Identity Federation según `docs/google-cloud-setup.md`; variables `GCP_*` cargadas en GitHub; región `us-central1` (ADR 0015). Pendiente: comprar o asignar el dominio (`ocpp.`, `api.`, `admin.`) antes de la iteración 8.
...
|---|---|---|---|---|
| 0 | Cimientos | Monorepo TypeScript (`apps/ocpp-gateway`, `apps/api`, `apps/worker`, `apps/backoffice`, `apps/mobile`, `packages/domain`, `packages/tariff-engine`, `packages/ocpp-schemas`, `packages/events`, `packages/db`, `packages/api-client`, `infra/terraform`), lint, tests, CI, Docker, entorno local con PostgreSQL y Redis, esquemas JSON oficiales de OCPP 1.6 | `pnpm test` y `pnpm lint` en verde en CI; contenedores construyen | [x] 19-09-2026: 72 pruebas en verde (dominio, esquemas, eventos, migración inicial contra PostgreSQL 16, tarifas, API, worker, gateway); primera ejecución de CI en GitHub Actions en verde, incluida la construcción de las tres imágenes Docker |
| 1 | Gateway OCPP mínimo | WebSocket `wss://…/ocpp/{chargeBoxId}` con subprotocolo `ocpp1.6`, Basic Auth con hash, allowlist, validación de esquema, `BootNotification` (`Pending`/`Accepted`), `Heartbeat`, `StatusNotification`, registro cargador → pod en Redis, ping/pong, logs correlacionados | Dos simuladores distintos se registran y reportan estado; conexión rechazada sin credenciales o con `chargeBoxId` desconocido; suite tzi-OCTT de registro en verde | [~] Hecho: handshake con allowlist y Basic Auth (usuario = identidad), rechazo sin subprotocolo, validación contra esquemas oficiales, `BootNotification` según ciclo de vida, `Heartbeat`, `StatusNotification` con detección de transiciones no previstas, `DataTransfer`, `Authorize` (Invalid), cierre de conexión duplicada, eventos de dominio, salud y métricas, cierre escalonado 1012, registro cargador → pod en Redis (`cs:conn`, TTL 90 s, refresco, compare-and-delete y evicción `cs:evict`, ADR 0013), restricción de acciones mientras el cargador está pendiente, log OCPP enmascarado; probado con el simulador embebido `@volt/ocpp-sim` y con un cliente OCPP-J crudo. Pendiente (necesita Docker en la máquina del dueño): prueba con los dos simuladores externos de `lab/` y suite tzi-OCTT en CI |
| 2 | Inventario y comisionamiento | Modelo de datos (DDL de `docs/sql`), API de sedes, estaciones, EVSE y conectores, ciclo de vida del cargador, plantillas de configuración por modelo, `GetConfiguration`/`ChangeConfiguration`, detección de deriva, `TriggerMessage`, `Reset`, `ChangeAvailability`, `UnlockConnector` | Un simulador pasa de inventariado a operativo desde la API; deriva detectada al cambiar una key | [x] 22-09-2026 (CI en verde, ejecución 35739422027): migración 0002 (ciclo de vida del dominio, credenciales, historial, tenant VOLT), `@volt/csms` (inventario, ciclo de vida, credenciales, comandos, comisionamiento, alarmas), `@volt/security` (scrypt, ADR 0014), `@volt/gateway-client` y API interna del gateway (ADR 0013), gateway con registro y persistencia en PostgreSQL, `/admin/v1` (sedes, plantillas, cargadores, credenciales, comisionamiento, sincronización, overrides, comandos, ciclo de vida, alarmas), trabajo diario de deriva en el worker, simulador embebido; prueba de aceptación `apps/api/src/commissioning.e2e.test.ts` en verde: INVENTORIED → PROVISIONED → CONNECTED_PENDING → CONFIGURED (con `RebootRequired` y `Reset`) → TESTED → OPERATIONAL y deriva detectada con alarma al cambiar `HeartbeatInterval` en el simulador. Pendiente de la iteración 3: sesión de prueba automática para pasar a TESTED |
| 3 | Sesiones | `RemoteStartTransaction`/`RemoteStopTransaction` por gRPC interno, `StartTransaction`, `StopTransaction`, `MeterValues`, máquinas de estado de conector y sesión, propiedad de la transacción, idempotencia, transacciones offline, outbox y Pub/Sub, SSE de progreso | Casos CU-02, CU-04 y CU-05 automatizados; corte de red simulado de 10 minutos sin duplicados; caída de pod con reconexión | [x] 22-09-2026: migración 0003, dominio de sesiones y transacciones en `@volt/csms` (ADR 0016), gateway con `Authorize`/`StartTransaction`/`MeterValues`/`StopTransaction`, API `/v1` con identidad de desarrollo y SSE sobre el outbox, rutas de administración de conductores y sesiones, worker con relay del outbox (Redis o log; Pub/Sub en la iteración 8), expiración de arranques, cierre de huérfanas y particiones mensuales; pruebas: `apps/api/src/sessions.e2e.test.ts` (CU-02 y CU-04 por la API con SSE, sesión de prueba → TESTED) y `apps/ocpp-gateway/src/transactions.db.test.ts` (CU-05 con corte de 10 minutos, cola offline, reintentos sin duplicados y caída de pod con reconexión a otro gateway). Pendiente para las iteraciones 4 y 5: costo en vivo y cobro; el adaptador de Pub/Sub llega con la iteración 8 |
| 4 | Motor de tarifas | Modelo OCPI, tarifas por franja horaria con precio por defecto, cotización previa, snapshot inmutable, cálculo incremental y final, idle fee con gracia de 15 minutos y 1.500 COP por minuto (ADR 0012, valores por defecto configurables), límite de exposición con `RemoteStopTransaction`, pruebas de propiedad, el ejemplo de TAR §6 y la tarifa base de Volt como fixtures | Ejemplo calculado reproducido al centavo; caso "carga termina, gracia hasta el minuto 15, ocupación cobrada por minuto" en verde; propiedades en verde; tope de exposición detiene la sesión en simulador | [x] Motor puro (`packages/tariff-engine`, 45 pruebas: ejemplo TAR §6 al centavo con 8,36 y 3,58 en curso, T1 a T12, seis propiedades con semilla fija, tarifa base de Volt); migración 0004 (`tariffs.*`, parámetros de precios y sesión); `@volt/csms` con tarifas versionadas, asignaciones con resolución explicable y fail-closed, parámetros por alcance, cotización, snapshot al autorizar, costo en curso con alertas, liquidación con líneas, recálculo idempotente y límites; API de administración y pública; worker (liquidación, límites, activación); ADR 0017. Aceptación en `apps/api/src/pricing.e2e.test.ts` |
| 5 | Pagos con Wompi (documentación: <https://docs.wompi.co/docs/colombia/inicio-rapido/>; llaves de prueba en secretos de GitHub desde el 22-09-2026) | Puerto `PaymentGateway` y adaptador Wompi: alta de fuente de pago con tokenización y 3DS, validación previa al inicio, cobro al cierre, webhooks con checksum e idempotencia, reintentos y bloqueo por deuda, enlace de pago de deuda, conciliación diaria, recibos; interfaz del adaptador DIAN | Flujo completo en sandbox de Wompi: alta de tarjeta, carga simulada, cobro aprobado, cobro rechazado con reintento y bloqueo, pago de deuda por enlace | [x] Paquete `@volt/payments` (puerto, adaptador Wompi, emulador, firma y checksum); migración 0006; `@volt/csms` con medios de pago, validación previa, cobro al liquidar, webhooks idempotentes, reintentos, deuda y enlace de pago, devolución, condonación, conciliación y recibos; API `/v1` (medios de pago, estado de cobro, deudas, recibos, webhook) y `/admin/v1` (pagos, deudas, bloqueos, conciliación); worker (ciclo de cobros y conciliación diaria); ADR 0020. Aceptación con el emulador en `apps/api/src/payments.e2e.test.ts` y prueba opcional contra el sandbox real en CI |
| 6 | Back-office | Autenticación con Identity Platform y MFA, RBAC (administrador, operaciones, soporte, lectura; `site_owner` definido), mapa y lista con estado en vivo, cargadores, sesiones, comandos, tarifas y simulador de precio, parámetros por nivel, pagos y deudas, alarmas, auditoría, bitácora por cargador, español e inglés | Todo comando y cambio de tarifa queda auditado; un operador comisiona un cargador en menos de 30 minutos | [x] 23-09-2026 (ADR 0021). Identity-Aware Proxy, *blocking functions* y exportación de la auditoría a un bucket pasan a la iteración 8 |
| 7 | App Volt v1 | React Native/Expo: registro con OIDC, alta de tarjeta con Wompi, mapa y lista con estado en vivo, precio desagregado visible antes de cargar, QR, inicio y parada, progreso en vivo (kWh, potencia, `SoC`, costo acumulado y tope), historial y recibos, deudas y pago, notificaciones push, español e inglés | Una carga completa en staging con el cargador real y el sandbox de Wompi de punta a punta; builds en TestFlight e Internal testing | [x] 23-09-2026 (ADR 0022): migración 0009, identidad del conductor y cuenta en `/v1`, notificaciones push en el worker, `apps/mobile` con Expo SDK 57; carga completa de punta a punta verificada en el navegador con la versión web de la app, el simulador embebido y el emulador de pagos (entrada, consentimientos, precio, alta de tarjeta, carga en vivo, parada, liquidación, cobro, recibo). Pendiente hasta tener cuentas: builds en TestFlight e Internal testing (Apple, Google Play y Expo); la carga en staging con el cargador real llega con la iteración 8 |
