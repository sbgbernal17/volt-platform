...
Estado al 23 de septiembre de 2026: **iteraciones 0, 2, 3, 4, 5 y 6 terminadas; iteración 1 casi cerrada** (hecho el registro cargador → pod en Redis; quedan la prueba con los dos simuladores externos del laboratorio, que necesita Docker, y la suite tzi-OCTT en CI); la siguiente es la iteración 7 (app Volt v1 con React Native/Expo). El repositorio contiene el diseño completo (`docs/`), este plan, el monorepo con los paquetes de dominio, esquemas OCPP, eventos, base de datos y tarifas, y las apps `ocpp-gateway`, `api` y `worker` con pruebas. Las tareas del dueño del proyecto están en `docs/tareas-del-dueno.md`. Este archivo es el plan vivo del proyecto: se actualiza al cerrar cada fase, al tomar cada decisión y cuando cambie el alcance.
