...
Estado al 22 de septiembre de 2026: **iteración 0 terminada y verificada en CI; iteración 1 en curso** (falta el registro cargador → pod en Redis, la prueba con los simuladores del laboratorio y la suite tzi-OCTT en CI); la siguiente es la iteración 2 (inventario y comisionamiento con la base de datos real). El repositorio contiene el diseño completo (`docs/`), este plan, el monorepo con los paquetes de dominio, esquemas OCPP, eventos, base de datos y tarifas, y las apps `ocpp-gateway`, `api` y `worker` con pruebas. Las tareas del dueño del proyecto están en `docs/tareas-del-dueno.md`. Este archivo es el plan vivo del proyecto: se actualiza al cerrar cada fase, al tomar cada decisión y cuando cambie el alcance.
