...
Estado al 22 de septiembre de 2026: **iteraciones 0, 2 y 3 terminadas; iteración 1 casi cerrada** (hecho el registro cargador → pod en Redis; quedan la prueba con los dos simuladores externos del laboratorio, que necesita Docker, y la suite tzi-OCTT en CI); la siguiente es la iteración 4 (motor de tarifas: franjas horarias, snapshot por sesión, costo en vivo, idle fee). El repositorio contiene el diseño completo (`docs/`), este plan, el monorepo con los paquetes de dominio, esquemas OCPP, eventos, base de datos y tarifas, y las apps `ocpp-gateway`, `api` y `worker` con pruebas. Las tareas del dueño del proyecto están en `docs/tareas-del-dueno.md`. Este archivo es el plan vivo del proyecto: se actualiza al cerrar cada fase, al tomar cada decisión y cuando cambie el alcance.
...
| 4 | Motor de tarifas | Modelo OCPI, tarifas por franja horaria con precio por defecto, cotización previa, snapshot inmutable, cálculo incremental y final, idle fee con gracia de 15 minutos y 1.500 COP por minuto (ADR 0012, valores por defecto configurables), límite de exposición con `RemoteStopTransaction`, pruebas de propiedad, el ejemplo de TAR §6 y la tarifa base de Volt como fixtures | Ejemplo calculado reproducido al centavo; caso "carga termina, gracia hasta el minuto 15, ocupación cobrada por minuto" en verde; propiedades en verde; tope de exposición detiene la sesión en simulador | [ ] |
