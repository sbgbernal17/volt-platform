...
- [x] 2026-09-25: la primera compilación de Android en EAS (build `f11af686`) falló en la fase de Gradle al empaquetar el JavaScript con Metro: `Cannot find module 'babel-preset-expo'` (con el `node_modules` aislado de pnpm, el `babel.config.js` de la app hacía que Babel buscara el preset donde no está). Arreglado: la app ya no lleva `babel.config.js` (Expo usa su preset por defecto) y se limpiaron los avisos de `expo doctor`. Nuevo flujo *Diagnóstico de compilación (EAS)* en Actions para leer el error de cualquier build sin entrar a expo.dev. Compilación relanzada con el arreglo y terminada bien (build `c6d57243`, versionCode 6, `.aab` de producción listo para descargar): los pasos de Play Console siguen en 0e.
- [x] 2026-09-25: primer `.aab` subido a Pruebas internas en Play Console (punto 0e, parcial): la app quedó enrolada en *Firma de apps de Play* y `eas submit` ya sirve para las siguientes subidas. La huella que me pasaste (`C2:D1:69:…:F1:92`) es la del certificado de clave de **subida** (la misma del keystore de EAS, ya incluida en la clave de Maps); falta la del certificado de clave de **firma de apps** (ver 0e).
...
| 0e | **Huella SHA-1 del certificado de clave de firma de apps de Google Play.** Ya subiste el primer `.aab`, así que Play Console → app Volt → Integridad de la app → Firma de apps muestra dos certificados. La huella que me pasaste (`C2:D1:69:78:AA:4E:7E:74:03:EB:07:48:AD:AE:E8:A8:DE:1D:F1:92`) es la del **certificado de clave de subida** (sección de abajo): coincide con el keystore de EAS y ya está en la clave de Maps. Falta la del **certificado de clave de firma de apps** (sección de arriba en esa misma página): es la clave con la que Google vuelve a firmar el paquete que instala la gente y normalmente es distinta. Si en tu consola las dos secciones muestran exactamente la misma huella, dímelo y no hay nada más que hacer | Sin ella el mapa no funciona en la versión que Google Play firma y distribuye; el resto de la app sí | La huella en el chat (no es secreta); relanzo el flujo *Clave de Google Maps (Android)* con las dos |
...
## Para la iteración 8 (infraestructura y observabilidad)
...
- [ ] **A dónde llegan las alarmas.** Un correo (y, si quieres, un teléfono para SMS) para los avisos P1/P2 de Cloud Monitoring: cargador fuera de línea, gateway caído, cobros fallando. Si prefieres avisos en el celular, instala la app *Google Cloud* con la cuenta de administrador y te añado como canal.
...
- [ ] **Orden y tamaño de los ambientes** (decisión con valor por defecto). Propongo levantar dev y staging primero y prod al final de la iteración con los tamaños mínimos (Cloud SQL sin alta disponibilidad hasta el primer cargador real). Orden de magnitud con precios de lista para prod con ~50 cargadores: USD 400-650/mes (ARQ §7); dev y staging bastante menos. Te doy la cifra por ambiente con el primer `terraform plan`, antes de crear nada que cueste; el presupuesto con alertas del 22-09 avisa si algo se sale. Si no dices nada, sigo con esa propuesta.
- [ ] **GitHub → Settings → Code security**: Dependabot alerts, secret scanning y push protection (2 minutos; es lo que te avisó con el JSON de Google Play, pero para todos los secretos).
