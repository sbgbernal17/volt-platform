...
| 0f | **Token de Expo para compilar desde GitHub Actions**: en <https://expo.dev> → tu cuenta → *Access tokens* → crea uno (nombre `github-actions`) y guárdalo en GitHub → Settings → Secrets and variables → Actions como secreto `EXPO_TOKEN` | El flujo *Compilación de la app (EAS)* del repositorio lanza compilaciones y envíos a las tiendas sin tu computador; así puedo compilar yo cuando haga falta | Solo confirma en el chat que el secreto está cargado |
