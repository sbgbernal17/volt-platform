...
| 0e | **Huella SHA-1 de la firma de Google Play.** Aparece solo después de la primera subida de un paquete: Play Console la genera al enrolar la app en *Firma de apps de Play*, y esa primera subida tiene que hacerse a mano en la consola (la API la rechaza). Pasos: (1) hecho: la compilación de Android con el perfil `production` ya terminó bien (build `c6d57243`, versionCode 6); (2) descarga el `.aab` en <https://expo.dev/accounts/supercargadores-vehiculares-sas/projects/volt-app/builds/c6d57243-da92-40df-9b4f-5200825973ca> (botón *Download*; hace falta tu sesión de Expo); (3) Play Console → app Volt → Pruebas → Pruebas internas → Crear versión → sube el `.aab` (acepta la firma administrada por Google, opción por defecto) → guarda; (4) Play Console → app Volt → Integridad de la app → Firma de apps → *Certificado de clave de firma de apps* → huella SHA-1 (no es secreta). Ese primer paquete es un marcador de posición: apunta a `api.supercargadores.co`, que existe con la iteración 8, así que la app no cargará datos todavía | Sin esa huella el mapa no funciona en la versión que Google Play vuelve a firmar; además la primera subida manual habilita `eas submit` para las siguientes | La huella en el chat; relanzo el flujo *Clave de Google Maps (Android)* con las dos huellas |
