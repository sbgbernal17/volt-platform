...
- [x] 2026-09-24: cuenta de Google Play lista; la de Apple en verificación (punto 0b). El acceso para compilar y publicar no es una invitación personal: se hace con credenciales de API guardadas en EAS (ver 0b abajo).
...
|---|---|---|---|
| 0b | **Acceso a las tiendas para compilar y publicar.** Apple: cuando App Store Connect termine la verificación, crea una clave de API en App Store Connect → Usuarios y acceso → Integraciones → Claves de API (rol *App Manager*), descarga el `.p8` y anota el *Key ID* y el *Issuer ID*; luego, en tu máquina, dentro de `apps/mobile`: `npx eas-cli credentials` → iOS → *App Store Connect API Key* → sube el `.p8` (EAS lo guarda cifrado; el archivo nunca va al repositorio). Google Play: Play Console → Usuarios y permisos → invita como usuario administrador la cuenta de servicio que se crea en Google Cloud (proyecto prod → IAM → Cuentas de servicio → nueva → clave JSON); ese JSON va a `apps/mobile/google-play-service-account.json` (ignorado por git) o como secreto de EAS. También crea la app en cada consola con el nombre "Volt" y el identificador `co.supercargadores.volt` | Compilar con EAS y subir a TestFlight e Internal testing sin que me des tu usuario personal | Con eso hecho, dime "listo" y yo dejo `eas.json` con `ascAppId` y `appleTeamId` y lanzo la primera compilación contigo |
