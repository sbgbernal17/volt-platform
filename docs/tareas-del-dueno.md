...
- [x] 2026-09-24: cuenta de Google Play lista, app Volt creada en Play Console y cuenta de servicio con clave JSON generada (se guarda cifrada en EAS, nunca en el repositorio); la cuenta de Apple sigue en verificación (punto 0b). El acceso para compilar y publicar no es una invitación personal: se hace con credenciales de API guardadas en EAS (ver 0b abajo).
...
|---|---|---|---|
| 0b | **Acceso a las tiendas para compilar y publicar.** Apple: cuando App Store Connect termine la verificación, crea una clave de API en App Store Connect → Usuarios y acceso → Integraciones → Claves de API (rol *App Manager*), descarga el `.p8` y anota el *Key ID* y el *Issuer ID*; luego, en tu máquina, dentro de `apps/mobile`: `npx eas-cli credentials` → iOS → *App Store Connect API Key* → sube el `.p8` (EAS lo guarda cifrado; el archivo nunca va al repositorio). Google Play (app Volt ya creada el 24-09-2026): la clave JSON de la cuenta de servicio **no se sube al repositorio ni se pega en el chat**: guárdala fuera de la carpeta del repositorio (o como `apps/mobile/google-play-service-account.json`, que git ignora) y súbela cifrada a EAS desde `apps/mobile`: `npx eas-cli credentials -p android` → perfil `production` → *Google Service Account* → *Set up a Google Service Account Key for Play Store Submissions* → elige el archivo; después puedes borrarlo. Además: en Google Cloud (el proyecto donde creaste la cuenta de servicio) habilita la API *Google Play Android Developer*, y en Play Console → Usuarios y permisos invita el correo de esa cuenta de servicio con acceso a la app Volt y permisos de publicación en pistas de prueba. Si la llave llegó a entrar en un commit, revócala en Google Cloud y genera otra. En App Store Connect falta crear la app "Volt" con el identificador `co.supercargadores.volt` cuando termine la verificación | Compilar con EAS y subir a TestFlight e Internal testing sin que me des tu usuario personal | Con eso hecho, dime "listo" y yo dejo `eas.json` con `ascAppId` y `appleTeamId` y lanzo la primera compilación contigo |
| 0d | **Términos y condiciones** de la app publicados (por ejemplo `supercargadores.co/terminos`) | La app los enlaza en consentimientos y en Cuenta; la política de datos ya está | La URL en el chat; la dejo como valor por defecto de `APP_TERMS_URL` |
| 0e | **Clave de Google Maps para Android: no me la pases por el chat ni por el repositorio.** Yo la creo y restrinjo con Terraform en la iteración 8 (API "Maps SDK for Android", paquete `co.supercargadores.volt` y huellas SHA-1). Las huellas salen después de la primera compilación: `npx eas-cli credentials` → Android → *Keystore* (SHA-1 del keystore de EAS) y Play Console → Configuración → Firma de apps (SHA-1 de la firma de Google Play). Si ya creaste una clave sin restricciones, bórrala o restríngela | El mapa de estaciones en Android (iOS usa Apple Maps sin clave) | Las dos huellas SHA-1 en el chat (no son secretas) cuando existan; la clave se inyecta en la compilación con la variable `GOOGLE_MAPS_ANDROID_API_KEY` de EAS |
