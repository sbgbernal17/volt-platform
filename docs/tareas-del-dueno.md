...
- [x] 2026-09-22: verificación de Google Cloud desde GitHub Actions con la identidad federada: los tres proyectos (`volt-dev-509415`, `volt-staging-509415`, `volt-prod-509415`) autentican y tienen la API de Identity Platform habilitada; el servicio aún no está inicializado (ver punto 0 de "Pendiente ahora").
...
|---|---|---|---|
| 0 | **Inicializar Identity Platform en dev y prod** (hoy solo está habilitada su API; el flujo *Verificación de Google Cloud* del 22-09-2026 devolvió `CONFIGURATION_NOT_FOUND` en los tres proyectos). En cada proyecto: Consola → Identity Platform → **Habilitar**; Proveedores → añadir **Correo electrónico/contraseña**; Configuración → Autenticación multifactor → **TOTP habilitado**; Usuarios → **Añadir usuario** con tu correo y una contraseña provisional | Entrar al back-office con tu correo (iteración 6) en dev y, más adelante, en producción | Cuando esté, dime "listo" y vuelvo a correr el flujo de verificación (Actions → Verificación de Google Cloud → Run workflow) o córrelo tú y mira el resumen |
| 0b | **Corregir los secretos del sandbox de Wompi** en GitHub (`WOMPI_PUBLIC_KEY_TEST` y `WOMPI_PRIVATE_KEY_TEST` no empiezan por `pub_test_` y `prv_test_`) | Que la prueba automática contra el sandbox real corra en cada push | Panel de Wompi → Desarrolladores → Llaves API → ambiente Sandbox; vuelve a guardar los cuatro secretos sin espacios |
